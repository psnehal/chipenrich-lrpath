...
chipenrich = function(
  ...
  if (!is.null(out_name)) {
    ...
    if (qc_plots) {
      	filename_qcplots = file.path(out_path,sprintf("%s_qcplots.jpeg",out_name));
        filename_disttotss = file.path(out_path,sprintf("%s_disttss.jpeg",out_name));

        jpeg(filename_qcplots);
        print(..plot_spline_length(ldef,peak_genes,num_peaks,mappa=mappa));
        dev.off();

        jpeg(filename_disttotss);
